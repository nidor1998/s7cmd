...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["put-object-tagging", "--help"]));
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("put-object-tagging"));
    assert!(!ok);
...
fn missing_tagging_flag_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().args(["put-object-tagging", "s3://bucket/key"]));
    assert!(!ok);
...
fn bucket_only_path_no_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s7cmd().args(["put-object-tagging", "s3://bucket", "--tagging", "k=v"]));
    assert!(!ok);
