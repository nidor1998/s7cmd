...
fn help_shows_both_positionals() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["put-bucket-cors", "--help"]));
...
fn missing_both_positionals_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("put-bucket-cors"));
    assert!(!ok);
...
fn missing_config_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().args(["put-bucket-cors", "s3://example-bucket"]));
    assert!(!ok);
    assert_eq!(code, Some(2), "stderr: {stderr}");
}
