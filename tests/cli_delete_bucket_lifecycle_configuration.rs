...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) =
        run(s7cmd().args(["delete-bucket-lifecycle-configuration", "--help"]));
    assert!(ok);
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("delete-bucket-lifecycle-configuration"));
    assert!(!ok);
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s7cmd().args(["delete-bucket-lifecycle-configuration", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
