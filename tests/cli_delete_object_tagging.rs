...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["delete-object-tagging", "--help"]));
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("delete-object-tagging"));
    assert!(!ok);
...
fn bucket_only_path_no_key_exits_1() {
    let (ok, _stdout, stderr, code) = run(s7cmd().args(["delete-object-tagging", "s3://bucket"]));
    assert!(!ok);
