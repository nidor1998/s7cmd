...
#[tokio::test]
async fn put_public_access_block_invalid_json_exits_1() {
    ...
    let local_dir = create_temp_dir();
    let config_path = create_test_file(&local_dir, "pab.json", b"not valid json");
    ...
    assert_eq!(
        code,
        Some(1),
        "put-public-access-block with invalid JSON must exit 1"
    );
    assert!(
