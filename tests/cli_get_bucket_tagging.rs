...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["get-bucket-tagging", "--help"]));
...
fn missing_positional_exits_2() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("get-bucket-tagging"));
    assert!(!ok);
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) = run(s7cmd().args(["get-bucket-tagging", "s3://bucket/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
